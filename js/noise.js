...
$(function () {
  ...
  function setSeqSpeed(v) { seqSpeedInterval=v; console.log(v); }
  ...
  function setOscType(v) { $wave.val(oscillator.type = v); console.log(v); }
  $wave.knob({ bgColor:"white", min:0, max:3, cursor:true, angleOffset:-140, angleArc:280, "change" : setOscType });
  setOscType(oscillator.SAWTOOTH); $wave.trigger("change");

  function setOscPitch(v) { 
    $pitch.val(oscillator.frequency.value = v); 
    var midiVal = freqToMidi(v);
    if($mpitch.val()!=midiVal){ $mpitch.val(midiVal).trigger("change"); } 
    console.log(v); 
  }
  ...
  function setMPitch(v) { 
    $mpitch.val(v); 
    oscillator.frequency.value = midiToFreq(v); 
    if($pitch.val()!=oscillator.frequency.value){ $pitch.val(oscillator.frequency.value).trigger("change"); } 
    console.log(v); 
  }
  $mpitch.knob({ bgColor:"white", min:0, max:127, angleOffset:-140, angleArc:280, "change" : setMPitch });
  setMPitch(36); $mpitch.trigger("change");

  function setFilterT(v) { $filtert.val(filter.type = v); console.log(v); }
  $filtert.knob({ bgColor:"white", min:0, max:7, cursor:true, angleOffset:-140, angleArc:280, "change" : setFilterT });
  setFilterT(filter.LOWPASS); $filtert.trigger("change");

  function setFilterF(v) { $filterf.val(filter.frequency.value = v); console.log(v); }
  ...
  function setFilterQ(v) { $filterq.val(v); filter.Q.value = v/10; console.log(v); }
  ...
  function setFilterG(v) { $filterg.val(filter.gain.value = v); console.log(v); }
  ...
  function setGain(v) { $gain.val(v); gainNode.gain.value = v/100; console.log(v); }
  ...
  function triggerOnce(){
    ...
    oscillatorTrig.type = $wave.val();
    oscillatorTrig.frequency.value = $pitch.val();
    
    var filterTrig = context.createBiquadFilter();
    filterTrig.type = $filtert.val();
    filterTrig.frequency.value = $filterf.val();
    filterTrig.Q.value = $filterq.val();
    filterTrig.gain.value = $filterg.val();
    ...
    filterTrig.gain.linearRampToValueAtTime(filter.frequency.value, timeAtAttack);
  ...
  function triggerRepeat(){
    ...
    var seqVal = (parseInt($("#s"+seqPos).attr("value"),10)+50)*5;
    console.log("seqVal:"+seqVal+" pos:"+seqPos);
    $pitch.val(seqVal).trigger("change");
